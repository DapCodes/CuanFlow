...
@section('title', 'Edit Kategori Permission')
@section('page-title', 'Edit Kategori Permission')
...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.permission-categories.update', $permissionCategory) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-900">Edit Kategori: {{ $permissionCategory->name }}</h2>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5 shadow-sm">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $permissionCategory->name) }}" required
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          ...
                          placeholder="Deskripsi singkat mengenai grup permission ini">{{ old('description', $permissionCategory->description) }}</textarea>
            </div>
        </div>
