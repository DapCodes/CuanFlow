    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Detail informasi produksi</p>
            </div>

            <div class="flex flex-wrap gap-3">
                @if($production->status === 'planned')
                    <form action="{{ route('production.start', $production->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin memulai produksi ini? Stok bahan baku akan langsung dikurangi.')">
                        @csrf
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium shadow-sm">
                            <i class="fas fa-play mr-2"></i>
                            Mulai Produksi
                        </button>
                    </form>
                    <button onclick="openCancelModal()" class="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium shadow-sm">
                ...
                @if($production->status === 'in_progress')
                    <button onclick="openCompleteModal()" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium shadow-sm">
                ...
                <a href="{{ route('production.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors font-medium shadow-sm">
...
<div id="completeModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full">
        <div class="bg-gradient-to-r from-green-500 to-emerald-500 p-6 rounded-t-xl">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Jumlah Aktual Produksi <span class="text-red-500">*</span>
                    </label>
                    <input type="number" step="0.01" name="actual_quantity" 
                        value="{{ $production->planned_quantity }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" 
                        placeholder="0" required min="0">
                    <p class="text-xs text-gray-500 mt-1">Jumlah produk yang berhasil diproduksi</p>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="waste_quantity" value="0"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" 
                        placeholder="0" min="0">
                    <p class="text-xs text-gray-500 mt-1">Produk yang rusak atau tidak memenuhi standar</p>
                </div>
        ...
        <form action="{{ route('production.cancel', $production->id) }}" method="POST">
            ...
            <div class="p-6">
                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
                    <div class="flex rounded-xl shadow-2xl max-w-md w-full">
        <div class="bg-gradient-to-r from-red-500 to-pink-500 p-6 rounded-t-xl">
            <h3 class="text-xl font-bold text-white flex items-center">
                <i class="fas fa-times-circle mr-3"></i>
                Batalkan Produksi
        ...
        <form action="{{ route('production.cancel', $production->id) }}" method="POST">
            ...
            <div class="p-6">
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-yellow-700">
                                @if($production->status === 'in_progress')
                                    Membatalkan produksi akan mengembalikan semua bahan baku yang telah digunakan ke dalam stok.
                                @else
                                    Membatalkan produksi akan menghapus rencana produksi ini.
                                @endif
                            </p>
                        </div>
                    </div>
                ...
                <p class="text-sm text-gray-600 mb-4">
                    Apakah Anda yakin ingin membatalkan produksi <strong>#{{ $production->batch_number }}</strong>?
                </p>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="flex justify-between text-sm mb-2">
                        ...
                        <span class="font-semibold text-gray-900">{{ number_format($production->planned_quantity, 2) }} {{ $production->product->unit->name ?? '' }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Status:</span>
                        <span class="font-semibold text-gray-900">
                            {{ $production->status === 'in_progress' ? 'Sedang Proses' : 'Direncanakan' }}
                        </span>
                    </div>
                </div>
            </div>
...
<script>
    ...
    // Calculate and display waste percentage in real-time
    ...
    function updateWastePercentage() {
        if (actualInput && wasteInput) {
            const actual = parseFloat(actualInput.value) || 0;
            const waste = parseFloat(wasteInput.value) || 0;
            const total = actual + waste;
            
            if (total > 0) {
                const wastePercent = (waste / total * 100).toFixed(1);
                const existingHint = wasteInput.parentElement.querySelector('.waste-percentage');
                
                if (waste > 0) {
                    if (existingHint) {
                        existingHint.textContent = `Waste: ${wastePercent}% dari total produksi`;
                        existingHint.className = 'waste-percentage text-xs mt-1 ' + 
                            (wastePercent > 10 ? 'text-red-600 font-semibold' : 'text-orange-600');
                    } else {
                        const hint = document.createElement('p');
                        hint.className = 'waste-percentage text-xs mt-1 ' + 
                            (wastePercent > 10 ? 'text-red-600 font-semibold' : 'text-orange-600');
                        hint.textContent = `Waste: ${wastePercent}% dari total produksi`;
                        wasteInput.parentElement.appendChild(hint);
                    }
    ...
    actualInput?.addEventListener('input', updateWastePercentage);
    wasteInput?.addEventListener('input', updateWastePercentage);
