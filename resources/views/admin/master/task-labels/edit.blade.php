...
@section('title', 'Edit Label Tugas')
@section('page-title', 'Edit Label Tugas')
...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.task-labels.update', $taskLabel) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-900">Edit Label: {{ $taskLabel->name }}</h2>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5 shadow-sm">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $taskLabel->name) }}" required
            ...
            <div>
                ...
                <div class="flex gap-2">
                    <input type="color" name="color" id="color" value="{{ old('color', $taskLabel->color) }}" 
                           class="h-12 w-12 border border-gray-300 rounded-lg cursor-pointer">
                    <input type="text" id="color-text" value="{{ old('color', $taskLabel->color) }}"
