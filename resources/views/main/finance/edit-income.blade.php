...
@section('title', 'Edit Pemasukan - ' . (auth()->user()->outlet->name ?? 'CuanFlow'))
...
<li class="flex items-center">
    ...
    <span class="text-gray-900 font-medium">Edit Pemasukan</span>
</li>
@endsection
...
<main class="flex-grow py-10 px-4 bg-gray-50">
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <section class="bg-white border border-gray-200 rounded-xl shadow-sm px-6 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Memperbarui data pemasukan lainnya untuk nomor <strong>{{ $expense->expense_number }}</strong>.
                </p>
            </div>
        ...
        {{-- Form --}}
        <form action="{{ route('finance.income.update', $expense->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="amount" id="amount" required step="0.01" min="0" value="{{ old('amount', abs($expense->amount)) }}"
                        ...
                        {{-- Date --}}
                        <div>
                            <label for="income_date" class="block text-sm font-bold text-gray-700 mb-2">Tanggal Terima <span class="text-red-500">*</span></label>
                            <input type="date" name="income_date" id="income_date" required value="{{ old('income_date', $expense->expense_date->format('Y-m-d')) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                class="w-full px-4 py-2.5 bg-gray-50 border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                                placeholder="Detail tambahan jika diperlukan...">{{ old('notes', $expense->notes) }}</textarea>
                        </div>
                    </div>
