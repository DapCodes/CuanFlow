...
class OfflineSyncQueue extends Model
{
    ...
    {
        $this->update(['status' => 'completed', 'synced_id' => $id, 'synced_at' => now()]);
    }

    public function markAsFailed(string $err): void
    {
        $this->update(['status' => 'failed', 'error_message' => $err, 'retry_count' => $this->retry_count + 1]);
    }
