...
<style>
    ...
    .generate-btn { background: linear-gradient(135deg, #d97706 0%, #b45309 100%); transition: all 0.3s ease; }
    .generate-btn:hover:not(:disabled) { box-shadow: 0 8px 24px -6px rgba(217, 119, 6, 0.4); transform: translateY(-1px); }
    .generate-btn:disabled { opacity: 0.5; cursor: not-allowed; }
    .tone-pill.active { background: #1f2937; color: white; }
    .lang-toggle.active { background: #d97706; color: white; }
...
<div x-data="generatorApp()" class="min-h-[calc(100vh-64px-57px)] bg-gray-50">
    ...
    <div class="bg-white border-b border-gray-200">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-base font-black text-gray-900 tracking-tight">Affiliate Script Generator</h1>
                    <p class="text-xs text-gray-400 font-medium mt-0.5">Buat script high-converting untuk TikTok, IG, YouTube</p>
                </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Deskripsikan script yang ingin dibuat</label>
            <textarea x-model="prompt" @keydown.ctrl.enter="submit()" @keydown.meta.enter="submit()"
                placeholder="Contoh: Script jualan takoyaki pedas yang viral di TikTok..."
                maxlength="2000" rows="3"
                class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 focus:bg-white text-sm resize-none transition-all"
        ...
        <div x-show="output || loading || error" class="fade-in">
            ...
            <div x-show="output && !loading">
                <div class="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5">
                    ...
                    <div class="output-area text-sm text-gray-800 leading-relaxed border-l-2 border-amber-300 pl-4" x-text="output"></div>
                </div>
            </div>
        </div>
        ...
        <div x-show="!output && !loading && !error" class="space-y-2">
            ...
            <button @click="prompt = 'Script jualan produk best seller kami untuk TikTok'; submit()"
                class="w-full text-left px-4 py-3 bg-white border border-gray-200 hover:border-amber-300 rounded-xl text-sm text-gray-600 hover:text-amber-700 transition-all">
                Script jualan produk best seller kami untuk TikTok
            </button>
            <button @click="prompt = 'Script review produk dengan gaya storytelling'; submit()"
                class="w-full text-left px-4 py-3 bg-white border border-gray-200 hover:border-amber-300 rounded-xl text-sm text-gray-600 hover:text-amber-700 transition-all">
                Script review produk dengan gaya storytelling
            </button>
            <button @click="prompt = 'Script promo diskon akhir bulan yang mendesak'; submit()"
                class="w-full text-left px-4 py-3 bg-white border border-gray-200 hover:border-amber-300 rounded-xl text-sm text-gray-600 hover:text-amber-700 transition-all">
