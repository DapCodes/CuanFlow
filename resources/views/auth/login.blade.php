...
<body class="bg-white">
                ...
                @else
                    ...
                    <form id="login-form" method="POST" action="{{ route('login') }}" class="space-y-4 xl:space-y-5">
                        ...
                        <div class="flex items-center justify-between pb-1 px-1">
                            ...
                            <a href="{{ route('password.request') }}" class="text-[11px] font-bold text-gray-400 hover:text-gray-900 transition-colors">Lupa Password? <span class="underline">Klik Disini</span></a>
                        </div>
    ...
    <script>
        ...
        @if (session('error_google'))
            window.addEventListener('DOMContentLoaded', () => showAlert('warning', 'Akun Tidak Ditemukan', '{{ session("error_google") }}'));
        @endif
    </script>
</body>
</html>