...
<li class="flex items-center gap-2">
    ...
    <span class="text-emerald-600 font-semibold tracking-wide text-sm">Kategori Permission</span>
</li>
@endsection
...
<div class="px-4 lg:px-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight uppercase">Kategori Permission</h1>
                <p class="text-sm text-gray-500 mt-0.5 font-medium italic">Pengelompokan fitur untuk manajemen hak akses role</p>
            </div>
        </div>
        <div>
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500">Perms Terkait</p>
                    <p class="mt-1 text-2xl font-black text-blue-600">{{ number_format($stats['total_assigned_permissions']) }}</p>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500">Kategori Kosong</p>
                    <p class="mt-1 text-2xl font-black text-red-600">{{ number_format($stats['empty_categories']) }}</p>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm">
            <div class="flex items-center justify-between">
                ...
                <div class="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center border border-emerald-100 shadow-sm shadow-emerald-100/50">
                    <i class="fas fa-calendar-plus text-emerald-500 text-lg"></i>
                </div>
            </div>
        </div>
    </section>
    ...
    <x-card-container class="!p-0 overflow-hidden border border-gray-200 shadow-sm bg-white rounded-xl">
        ...
        <div class="border-b border-gray-200 px-4 md:px-6 py-5 bg-gray-50/50">
            <form action="{{ route('admin.permission-categories.index') }}" method="GET" class="space-y-4 md:space-y-0 md:flex md:items-center md:justify-between gap-4">
                <div class="w-full md:max-w-xs">
                    <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2 block italic">Cari Nama Kategori / Slug</label>
                    <div class="relative group">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Contoh: Akuntansi..."
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->anyFilled(['search']))
                        <a href="{{ route('admin.permission-categories.index') }}" class="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-white border border-gray-200 text-gray-400 hover:bg-gray-50 hover:text-red-500 transition-all shadow-sm active:scale-95" title="Reset">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50/50 text-gray-400 text-[10px] font-bold uppercase tracking-widest border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left">Slug/Identifikasi</th>
                        <th class="px-6 py-4 text-center">Data Terkait</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-5">
                            <div class="flex items-center gap-4">
                                <div class="w-11 h-11 rounded-xl flex items-center justify-center shadow-sm" 
                                     style="background-color: {{ $category->color ?? '#10b981' }}15; border: 1px solid {{ $category->color ?? '#10b981' }}30;">
                                    <i class="{{ $category->icon ?? 'fas fa-tags' }} text-xs" style="color: {{ $category->color ?? '#10b981' }};"></i>
                                </div>
                                <div>
                        ...
                        <td class="px-6 py-5 text-center">
                            <div class="flex flex-col items-center">
                                <span class="text-[11px] font-black {{ $category->permissions_count > 0 ? 'text-gray-900' : 'text-red-400' }}">
                                    {{ number_format($category->permissions_count) }}
                                </span>
                        ...
                        <td class="px-6 py-5">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                @else
                                    <div class="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-gray-300 border border-gray-100 cursor-not-allowed" title="Kategori masih memiliki data">
                        ...
                        <td colspan="4" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-[11px] text-gray-500 font-bold uppercase tracking-widest mt-2 max-w-xs mx-auto italic">
                                    {{ request('search') ? 'Coba sesuaikan kata kunci pencarian Anda.' : 'Daftar kategori masih kosong.' }}
                                </p>
                            </div>
                        </td>
                    </tr>
