...
@section('title', 'Arsip System Error Log')
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Arsip System Error Log</h1>
                <p class="text-sm text-gray-500 mt-0.5 font-medium">Koleksi log error sistem yang telah di-backup dari file utama</p>
            </div>
        </div>
        <a href="{{ route('admin.security.error-logs.index') }}" 
           class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-all duration-200 shadow-sm">
            ...
            <span>Kembali ke Log Error Aktif</span>
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 flex items-center gap-3 bg-blue-50/30 border-blue-100">
            ...
            <p class="text-xs text-blue-700 leading-normal">System Error Log lama perlu di-backup untuk mencegah ukuran file <code class="bg-blue-100 px-1 rounded">laravel.log</code> terlalu besar dan memperlambat server.</p>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                @if($archive->status === 'completed')
                                    <a href="{{ route('admin.security.error-logs.archives.view', $archive->id) }}" 
                                       class="inline-flex items-center justify-center w-8 h-8 text-blue-500 hover:bg-blue-50 rounded-lg transition-all" 
                                       title="Lihat JSON">
                                        <i class="fas fa-eye text-sm"></i>
                                    </a>
                                    <a href="{{ route('admin.security.error-logs.archives.download', $archive->id) }}" 
