...
<div class="px-4 lg:px-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight uppercase">Kelola Admin</h1>
                <p class="text-sm text-gray-500 mt-0.5 font-medium italic">Manajemen akun administrator dengan hak akses penuh sistem</p>
            </div>
        </div>
        <div>
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500">Total Admin</p>
                    <p class="mt-1 text-2xl font-black text-gray-900">{{ number_format($stats['total_admins']) }}</p>
                </div>
                <div class="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center border border-gray-100">
                    <i class="fas fa-user-shield text-gray-400 text-lg"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500">Admin Aktif</p>
                    <p class="mt-1 text-2xl font-black text-emerald-600">{{ number_format($stats['active_admins']) }}</p>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-2xl font-black text-red-600">{{ number_format($stats['inactive_admins']) }}</p>
                </div>
    ...
    <x-card-container class="!p-0 overflow-hidden border border-gray-200 shadow-sm bg-white rounded-xl">
        ...
        <div class="border-b border-gray-200 px-4 md:px-6 py-5 bg-gray-50/50">
            <form action="{{ route('admin.admins.index') }}" method="GET" class="space-y-4 md:space-y-0 md:flex md:items-center md:justify-between gap-4">
                <div class="w-full md:max-w-xs">
                    <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2 block italic">Cari Nama / Email</label>
                    <div class="relative group">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Contoh: Admin Utama..."
                ...
                <div class="flex flex-wrap items-end gap-3 w-full md:w-auto">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->anyFilled(['search', 'status']))
                            <a href="{{ route('admin.admins.index') }}" class="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-white border border-gray-200 text-gray-400 hover:bg-gray-50 hover:text-red-500 transition-all shadow-sm active:scale-95" title="Reset">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50/50 text-gray-400 text-[10px] font-bold uppercase tracking-widest border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left">Email & Keamanan</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-5">
                            <div class="flex items-center gap-4">
                                <div class="w-11 h-11 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-600 border-2 border-emerald-100 shadow-sm font-black text-sm">
                                    {{ substr($admin->name, 0, 1) }}
                                </div>
                                <div>
                                    <p class="font-black text-gray-900 leading-tight uppercase tracking-tight">{{ $admin->name }}</p>
                                    <p class="text-[10px] font-black uppercase tracking-widest text-emerald-500 mt-1 italic">SYSTEM ADMINISTRATOR</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-5">
                            <div class="text-[11px] font-bold text-gray-900">{{ $admin->email }}</div>
                            <div class="flex items-center gap-1.5 mt-1 text-emerald-600">
                                <i class="fas fa-shield-check text-[9px]"></i>
                                <span class="text-[9px] font-black uppercase tracking-widest">Verified System Admin</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                @else
                                    <div class="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-gray-300 border border-gray-100 cursor-not-allowed" title="Proteksi Root">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-base font-black text-gray-900 uppercase tracking-widest">Admin Tidak Ditemukan</h3>
                                <p class="text-[11px] text-gray-500 font-bold uppercase tracking-widest mt-2 max-w-xs mx-auto italic">
                                    {{ request('search') ? 'Coba sesuaikan kata kunci pencarian Anda.' : 'Daftar admin masih kosong.' }}
                                </p>
                            </div>
                        </td>
                    </tr>
