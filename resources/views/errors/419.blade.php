...
<head>
    ...
    <title>419 - Sesi Berakhir | CuanFlow</title>
    ...
    <style>
        ...
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(49, 105, 78, 0.2); }
        .green-blur { background: radial-gradient(circle at top left, rgba(49, 105, 78, 0.15) 0%, transparent 70%); filter: blur(60px); }
        ...
        .error-code { font-size: 8rem; font-weight: 900; background: linear-gradient(135deg, #31694E 0%, #658C58 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; line-height: 1; }
        @media (max-width: 640px) { .error-code { font-size: 5rem; } }
    </style>
    <script>
        tailwind.config = { theme: { extend: { colors: { 'cuan-yellow': '#F0E491', 'cuan-olive': '#BBC863', 'cuan-green': '#658C58', 'cuan-dark': '#31694E', } } } }
    </script>
</head>
<body class="min-h-screen bg-white relative overflow-hidden flex items-center justify-center">
    ...
    <div class="relative z-10 w-full max-w-2xl px-6 sm:px-8 py-12 text-center">
        <div class="mb-8 animate-on-load animate-scale-in flex justify-center animate-fade-in-up delay-100">
            <img src="{{ asset('assets/image/full-logo.svg') }}" alt="CuanFlow Logo" class="w-full max-w-[140px] sm:max-w-[160px] h-auto" />
        </div>
        <div class="mb-6 animate-on-load animate-shake delay-100">
            <h1 class="error-code">419</h1>
        </div>
        <h2 class="text-3xl sm:text-4xl font-bold text-gray-900 mb-4 animate-on-load animate-fade-in-up delay-200">Sesi Berakhir</h2>
        <p class="text-gray-600 text-sm sm:text-base leading-relaxed mb-10 max-w-lg mx-auto animate-on-load animate-fade-in-up delay-300">
            Halaman ini telah kedaluwarsa karena Anda terlalu lama tidak aktif. Silakan segarkan halaman atau login kembali untuk melanjutkan aktivitas Anda.
        </p>
        <div class="flex flex-col sm:flex-row gap-3 justify-center animate-on-load animate-fade-in-up delay-400">
            <button onclick="window.location.reload()" class="btn-primary px-8 py-3.5 bg-cuan-dark text-white font-semibold rounded-lg text-base">Refresh Halaman</button>
            <a href="{{ route('login') }}" class="px-8 py-3.5 bg-white text-gray-700 font-medium rounded-lg border-2 border-gray-200 text-base hover:bg-gray-50 transition-colors">Login Kembali</a>
        </div>
