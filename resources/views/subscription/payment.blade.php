    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="p-8">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="font-bold text-lg text-gray-900">{{ $plan->tier->display_name }} ({{ $plan->duration_months ?? 1 }} Bulan)</h3>
                    <p class="text-sm text-gray-500">{{ $plan->tier->description }}</p>
                </div>
                <div class="text-right">
                    <span class="block text-2xl font-bold text-indigo-600">Rp {{ number_format($amount, 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div class="mt-8">
                <button id="pay-button" class="w-full bg-indigo-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-200">
