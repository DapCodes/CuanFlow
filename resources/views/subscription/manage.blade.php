        ...
        <div class="max-w-6xl mx-auto space-y-8">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div class="animate-fade-in-down">
                    ...
                    <p class="text-sm text-gray-500 font-medium mt-1">Lihat status langganan, perpanjang durasi, atau
                        upgrade tier akun Anda.</p>
                </div>
            </div>
                ...
                <aside class="lg:col-span-3 space-y-4">
                    ...
                    <nav class="hidden lg:flex flex-col gap-1.5 p-2 bg-white border border-gray-200 rounded-2xl shadow-sm">
                        <button @click="activeTab = 'overview'"
                            :class="activeTab === 'overview' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all font-bold text-sm text-left group">
                            <i class="fas fa-chart-pie text-lg opacity-40 group-hover:opacity-100 transition-opacity"
                                :class="activeTab === 'overview' ? 'opacity-100' : ''"></i>
                            Ringkasan & Perpanjang
                        </button>
                        <button @click="activeTab = 'upgrade'"
                            :class="activeTab === 'upgrade' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all font-bold text-sm text-left group">
                            <i class="fas fa-rocket text-lg opacity-40 group-hover:opacity-100 transition-opacity"
                                :class="activeTab === 'upgrade' ? 'opacity-100' : ''"></i>
                            Upgrade Tier
                        </button>
                    </nav>
                    ...
                    <nav
                        class="lg:hidden flex border border-gray-200 rounded-2xl bg-white p-1 overflow-x-auto no-scrollbar scroll-smooth shadow-sm">
                        <button @click="activeTab = 'overview'"
                            :class="activeTab === 'overview' ? 'bg-gray-100 text-gray-900 shadow-sm' : 'text-gray-500'"
                            class="flex-1 whitespace-nowrap px-4 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2">
                            <i class="fas fa-chart-pie"></i> Ringkasan
                        </button>
                        <button @click="activeTab = 'upgrade'"
                            :class="activeTab === 'upgrade' ? 'bg-gray-100 text-gray-900 shadow-sm' : 'text-gray-500'"
                            class="flex-1 whitespace-nowrap px-4 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2">
                    ...
                    {{-- Help Card --}}
                    <div
                        class="hidden lg:block p-6 bg-gradient-to-br from-cuan-dark to-cuan-green rounded-2xl shadow-xl text-white relative overflow-hidden group">
                        <div class="relative z-10">
                            <h4 class="text-sm font-bold mb-2">Butuh Bantuan?</h4>
                            <p class="text-[11px] text-white/80 leading-relaxed font-medium mb-4">Hubungi tim support kami
                                jika Anda memiliki pertanyaan mengenai tagihan.</p>
                            <a href="#"
                                class="inline-flex items-center text-[10px] font-black uppercase tracking-widest text-white/50 hover:text-white transition-colors">
                                Hubungi Kami <i
                    ...
                    <section x-show="activeTab === 'overview'" class="animate-fade-in-up" x-cloak>
                        <div class="bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden mb-6">
                            <div class="p-6 md:p-8">
                                <div class="flex items-center gap-4 mb-8">
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-cuan-yellow/20 text-cuan-dark flex items-center justify-center text-xl shadow-sm border border-cuan-yellow/50">
                                        <i class="fas fa-crown"></i>
                                    </div>
                                    <div>
                                        <h2 class="text-xl font-black text-gray-900">Status Langganan</h2>
                                        <p class="text-xs text-gray-500 font-medium mt-0.5">Informasi paket aktif dan masa
                                            berlaku Anda.</p>
                                    </div>
                                </div>
                                ...
                                {{-- Current Plan Card --}}
                                <div
                                    class="rounded-2xl bg-gradient-to-r from-gray-900 to-gray-800 text-white p-6 md:p-8 relative overflow-hidden shadow-lg">
                                    <div
                                        class="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-white opacity-5 rounded-full blur-3xl">
                                    </div>
                                    <div class="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                        <div>
                                            <div
                                                class="flex items-center space-x-2 text-cuan-yellow font-bold text-xs uppercase tracking-widest mb-2">
                                        ...
                                        <div class="md:text-right">
                                            <button
                                                @click="document.getElementById('extension-plans').scrollIntoView({behavior: 'smooth'})"
                                                class="px-6 py-3 bg-cuan-yellow text-cuan-dark font-black text-xs uppercase tracking-[0.2em] rounded-xl hover:bg-white transition-colors shadow-lg">
                                                Tambah Durasi
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div id="extension-plans" class="space-y-4">
                            <div class="flex items-center justify-between px-2">
                                ...
                                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">Tier
                                    {{ $currentTier->display_name }}</span>
                            </div>
                                ...
                                @foreach($extensionPlans as $plan)
                                    <div class="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-lg hover:border-cuan-green/30 transition-all group relative cursor-pointer"
                                        ...
                                        @if($plan->discount_percentage > 0)
                                            <div
                                                class="absolute -top-3 -right-3 bg-red-500 text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg rotate-12 group-hover:rotate-0 transition-transform">
                                                Hemat {{ number_format($plan->discount_percentage, 0) }}%
                                            </div>
                                        @endif

                                        <div
                                            class="absolute top-4 right-4 text-gray-300 group-hover:text-cuan-green transition-colors">
                                            <i class="fas fa-circle-plus text-xl"></i>
                                        </div>
                                        <p class="text-gray-500 text-xs font-bold uppercase tracking-wider mb-2">
                                            {{ $plan->duration_months }} Bulan</p>

                                        <div class="mb-4">
                                            @if($plan->discount_percentage > 0)
                                                <p class="text-xs text-gray-400 line-through font-medium">Rp
                                                    {{ number_format($plan->original_price, 0, ',', '.') }}</p>
                                            @endif
                                        ...
                                        <button
                                            class="w-full py-3 rounded-xl bg-gray-50 text-gray-900 text-xs font-black uppercase tracking-widest group-hover:bg-cuan-dark group-hover:text-white transition-colors">
                                            Pilih Paket
                                        </button>
                                    </div>
                                @endforeach
                    ...
                    <section x-show="activeTab === 'upgrade'" class="animate-fade-in-up" x-cloak>
                        <div class="bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden">
                            <div class="p-6 md:p-8">
                                <div class="flex items-center gap-4 mb-8">
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-purple-50 text-purple-600 flex items-center justify-center text-xl shadow-sm border border-purple-100/50">
                                        <i class="fas fa-rocket"></i>
                                    </div>
                                    <div>
                                        <h2 class="text-xl font-black text-gray-900">Upgrade Tier</h2>
                                        <p class="text-xs text-gray-500 font-medium mt-0.5">Dapatkan fitur lebih lengkap
                                            dengan beralih ke tier yang lebih tinggi.</p>
                                    </div>
                                </div>
                                ...
                                @else
                                    <div class="space-y-6">
                                        @foreach($upgradeTiers as $tier)
                                            <div
                                                class="border border-gray-200 rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
                                                <div
                                                    ...
                                                    <span
                                                        class="bg-purple-100 text-purple-700 text-[10px] font-black uppercase px-2 py-1 rounded tracking-wider">Upgrade</span>
                                                </div>
                                                <div class="p-6">
                                                    <p class="text-gray-600 text-sm mb-6 leading-relaxed">{{ $tier->description }}
                                                    </p>

                                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                        @foreach($tier->plans as $plan)
                                                                                        <button
                                                                                            onclick="confirmUpgrade({{ $plan->id }}, '{{ $tier->display_name }}', {{ $plan->price }}, {{ $plan->duration_months }}, {{ json_encode($tier->features->map(function ($f) {
                                                            return ['name' => $f->display_name ?? $f->name, 'description' => $f->description, 'icon' => $f->icon]; })) }})"
                                                                                            ...
                                                                                            @if($plan->discount_percentage > 0)
                                                                                                <div
                                                                                                    class="absolute -top-2 -right-2 bg-purple-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded shadow-sm z-10">
                                                                                                    -{{ number_format($plan->discount_percentage, 0) }}%
                                                                                                </div>
                                                                                            @endif

                                                                                            <span
                                                                                                class="text-xs font-bold text-gray-500 uppercase tracking-wider group-hover:text-purple-700">{{ $plan->duration_months }}
                                                                                                Bulan</span>

                                                                                            <div class="mt-1">
                                                                                                @if($plan->discount_percentage > 0)
                                                                                                    <span
                                                                                                        class="text-[10px] text-gray-400 line-through block leading-none">Rp
                                                                                                        {{ number_format($plan->original_price, 0, ',', '.') }}</span>
                                                                                                @endif
                                                                                            ...
                                                                                            <div
                                                                                                class="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity text-purple-600">
