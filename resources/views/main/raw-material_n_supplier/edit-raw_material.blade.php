...
<main class="flex-grow py-8 px-4">
    <div class="max-w-7xl mx-auto">
        ...
        <x-card-container>
            ...
            <!-- Form -->
            <form action="{{ route('raw-materials.update', $rawMaterial) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea name="description" 
                                          id="description" 
                                          rows="4"
                                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 @error('description') border-red-500 @enderror">{{ old('description', $rawMaterial->description) }}</textarea>
                                @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
