...
<div class="flex bg-gray-50" style="height: calc(100vh - 64px - 57px);">
    ...
    <div id="chatSidebar" class="w-64 bg-white border-r border-gray-200 flex flex-col fixed lg:relative z-30 h-full lg:translate-x-0 -translate-x-full transition-transform duration-300 ease-in-out">
        ...
        <div class="p-3 border-b border-gray-200 flex-shrink-0">
            <button onclick="createNewChat()" class="w-full px-4 py-2.5 bg-gradient-to-r from-indigo-500 to-blue-600 text-white rounded-lg hover:from-indigo-600 hover:to-blue-700 transition-all duration-200 text-sm font-medium shadow-sm hover:shadow">
        ...
        <div class="flex-1 overflow-y-auto p-2" id="chatHistory">
            ...
            <button onclick="loadChat({{ $s->id }})" 
                ...
                <div class="text-sm font-medium truncate {{ $s->id == $session->id ? 'text-indigo-700' : 'text-gray-700' }}">
                    {{ $s->title }}
                </div>
    ...
    <div id="sidebarOverlay" class="fixed inset-0 bg-black bg-opacity-50 z-20 lg:hidden hidden transition-opacity duration-300" onclick="toggleSidebar()"></div>
        ...
        <div class="bg-white border-b border-gray-200 px-4 py-3 flex-shrink-0">
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <button onclick="toggleSidebar()" class="lg:hidden text-gray-600 hover:text-gray-900 flex-shrink-0 p-2 -ml-2 hover:bg-gray-100 rounded-lg transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
                        </svg>
                    </button>
                    
                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span class="text-sm font-semibold text-indigo-600" id="quotaDisplay">{{ auth()->user()->daily_chat_quota }}</span>
                ...
                @empty
                    <div class="flex items-center justify-center h-full">
                        <div class="text-center max-w-md w-full">
                            ...
                            <p class="text-gray-600 text-sm mb-8">Saya Clara AI, siap membantu bisnis Anda dengan analisis data dan insight berharga</p>
                            
                            <div class="space-y-2">
                                <button onclick="askQuestion('Bagaimana trend penjualan minggu ini?')" class="w-full px-4 py-3 bg-white border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl text-sm text-left transition-all duration-200 shadow-sm hover:shadow group">
                                    <span class="text-gray-700 group-hover:text-indigo-700">Trend penjualan minggu ini</span>
                                </button>
                                <button onclick="askQuestion('Produk apa yang paling laris?')" class="w-full px-4 py-3 bg-white border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl text-sm text-left transition-all duration-200 shadow-sm hover:shadow group">
                                    <span class="text-gray-700 group-hover:text-indigo-700">Produk terlaris</span>
                                </button>
                                <button onclick="askQuestion('Produk mana yang stoknya menipis?')" class="w-full px-4 py-3 bg-white border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl text-sm text-left transition-all duration-200 shadow-sm hover:shadow group">
