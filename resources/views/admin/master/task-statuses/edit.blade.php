...
@section('title', 'Edit Status Tugas')
@section('page-title', 'Edit Status Tugas')

@section('breadcrumb')
<li class="flex items-center">
    <i class="fas fa-chevron-right mx-2 text-gray-400 text-xs"></i>
    <a href="{{ route('admin.task-statuses.index') }}" class="text-gray-500 hover:text-gray-700">Status Tugas</a>
</li>
<li class="flex items-center">
    <i class="fas fa-chevron-right mx-2 text-gray-400 text-xs"></i>
...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.task-statuses.update', $taskStatus) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-900">Edit Status: {{ $taskStatus->name }}</h2>
            <a href="{{ route('admin.task-statuses.index') }}" class="text-gray-500 hover:text-gray-700">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5 shadow-sm">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $taskStatus->name) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="color" name="color" id="color" value="{{ old('color', $taskStatus->color) }}" 
                               class="h-12 w-12 border border-gray-300 rounded-lg cursor-pointer">
                        <input type="text" id="color-text" value="{{ old('color', $taskStatus->color) }}"
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $taskStatus->order) }}" required
