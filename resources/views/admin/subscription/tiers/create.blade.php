...
@section('page-title', 'Tambah Paket Berlangganan Baru')
...
<li class="flex items-center gap-2">
    <i class="fas fa-chevron-right text-[10px] text-gray-300"></i>
    <span class="text-emerald-600 font-semibold tracking-wide text-sm">Tambah Paket</span>
</li>
@endsection

@section('content')
<form action="{{ route('admin.subscription-tiers.store') }}" method="POST" class="space-y-6 max-w-5xl mx-auto">
    @csrf
                ...
                <div class="p-8 space-y-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
                                  placeholder="Jelaskan keunggulan paket ini...">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" name="max_outlets" value="{{ old('max_outlets') }}"
                                   class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
                                   placeholder="Kosongkan untuk unlimited">
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-sm font-semibold text-gray-700">Trial Days</label>
                            <input type="number" name="trial_duration_days" value="{{ old('trial_duration_days', 30) }}"
                                   class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="mb-8 last:mb-0">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach($categoryFeatures as $feature)
                                    <label class="group flex items-start gap-3 p-4 rounded-2xl border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all cursor-pointer">
                                        <div class="mt-0.5">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm sticky top-6">
                ...
                <div class="mt-6 border-t border-gray-100 pt-6">
                    <div class="flex items-start gap-3 text-amber-600 bg-amber-50 p-4 rounded-xl border border-amber-100">
                        <i class="fas fa-circle-info mt-1"></i>
                        <p class="text-xs leading-relaxed font-medium">
                            <strong>Penting:</strong> Setelah membuat tier, Anda harus menambahkan <strong>Duration Plan</strong> agar user bisa memilih masa aktif (misal: 1 bulan, 12 bulan).
                        </p>
                    </div>
                </div>
            </div>
