...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.permissions.store') }}" method="POST" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-900">Tambah Permission Baru</h2>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            <div>
                ...
                <select name="permission_category_id" id="permission_category_id"
                    ...
                    <option value="{{ $category->id }}" {{ old('permission_category_id') == $category->id ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cuan-green"
                          placeholder="Deskripsi singkat permission ini">{{ old('description') }}</textarea>
            </div>
        </div>
