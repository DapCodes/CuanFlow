...
@section('title', 'Edit Kebijakan - ' . (auth()->user()->outlet->name ?? 'CuanFlow'))
...
<li class="flex items-center text-sm">
    <span class="text-gray-400 mx-2">/</span>
    <span class="text-gray-900 font-medium tracking-tight">Edit Kebijakan</span>
</li>
@endsection
    ...
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <section class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div class="animate-fade-in-down">
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Edit Kebijakan</h1>
                <p class="text-sm text-gray-500 font-medium mt-1">Perbarui detail prosedur atau aturan outlet Anda.</p>
            </div>
        ...
        {{-- Form Container --}}
        <form action="{{ route('outlet-policies.update', $outletPolicy->id) }}" method="POST" class="animate-fade-in-up">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                <div class="p-6 md:p-12 space-y-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $outletPolicy->title) }}" required placeholder="Contoh: Prosedur Pembukaan Kasir"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="text" name="category" id="category" value="{{ old('category', $outletPolicy->category) }}" placeholder="Contoh: Operasional, SDM, Keuangan"
                                class="w-full px-5 py-4 bg-gray-50 border-gray-100 rounded-xl text-sm font-bold text-gray-900 focus:ring-4 focus:ring-cuan-green/10 focus:border-cuan-green focus:bg-white transition-all placeholder:text-gray-300">
                        ...
                        <div class="bg-gray-50 rounded-2xl p-6 flex items-start gap-4 border border-gray-100">
                            ...
                            <div>
                                <h4 class="text-[11px] font-black uppercase tracking-wider text-gray-600 mb-1">Informasi Update</h4>
                                <p class="text-[10px] text-gray-400 font-medium leading-relaxed">
                                    Pembaruan terakhir dilakukan {{ $outletPolicy->updated_at->diffForHumans() }}.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="content" id="content" rows="12" required placeholder="Tuliskan detail prosedur atau aturan di sini..."
                            class="w-full px-5 py-4 bg-gray-50 border-gray-100 rounded-xl text-sm font-bold text-gray-900 focus:ring-4 focus:ring-cuan-green/10 focus:border-cuan-green focus:bg-white transition-all placeholder:text-gray-300 resize-none">{{ old('content', $outletPolicy->content) }}</textarea>
